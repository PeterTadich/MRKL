/*
var thetastart = [[1],[0],[0],[1],[1],[0.2],[0],[1]];
var thetaend = [[1.2],[0.5],[0.6],[1.1],[2],[2],[0.9],[1]];
var Tf = 4;
var N = 6;
var method = 3;
...
function JointTrajectory(thetastart, thetaend, Tf, N, method){
    ...
    for(var i=1;i<=N;i=i+1){
        if(method === 3){
            var s = CubicTimeScaling(Tf, timegap * (i - 1));
        } else {
            var s = QuinticTimeScaling(Tf, timegap * (i - 1));
        }
        traj[i-1] = matrix_arithmetic(thetastart,matrix_multiplication_scalar(matrix_arithmetic(thetaend,thetastart,"-"),s),"+");
    }
    //traj = matrix_transpose(traj);
    return traj;
}
